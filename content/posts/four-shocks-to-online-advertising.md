---
title: 'Four Shocks to Online Advertising: From Real-Time Bidding to Machine Audiences'
date: '2026-08-10'
draft: false
tags:
- online advertising
...
I didn't discover this. A developer ran the same test in early August and wrote it up ([Schmalbach, 2026](https://www.vincentschmalbach.com/time-serves-ai-bots-a-different-website/); [Digiday, 2026](https://digiday.com/media/time-has-started-serving-ads-to-ai-agents/)); I went to verify it for myself. The verification held. But I also tested nine other publishers, and that part turned out to be more interesting than the original finding.
...
HTTP 402 is Payment Required. The HTTP/1.1 spec marked it "reserved for future use," and for nearly thirty years it had no serious deployment. It is now in production — from two different vendors.

I will come back to all of this. The short version is that these findings point at one thing: a premise the online advertising industry hasn't had to question in fifteen years: that there is a human at the end of the chain.
...
Two conditions made this work.

The first was the auction. Second-price auctions were standard: the winner pays the second-highest price. That mechanism has a well-known property. Under idealized conditions of a single item, private values, and no budget constraint, bidding your true valuation is optimal.
...
The published work from that period concentrates on this question. Google published the internals of its production CTR system in 2013: FTRL, memory savings, and, notably, calibration of predicted probabilities ([McMahan et al., 2013](https://research.google.com/pubs/archive/41159.pdf)). Facebook published its boosted-trees-plus-logistic-regression pipeline in 2014 ([He et al., 2014](https://dl.acm.org/doi/10.1145/2648584.2648589)). The same year, Criteo addressed a related problem: conversions arrive hours or days after the click, so your training negatives contain positives that haven't happened yet. Their fix was to model conversion probability and the delay distribution jointly ([Chapelle, 2014](http://wnzhang.net/share/rtb-papers/delayed-feedback.pdf)).

A second class of problem appeared in the same period. Give an advertiser a daily budget and, uncontrolled, the system spends it in the first hour on morning traffic. The most efficient advertisers also exhaust budget first and drop out, which reduces publisher revenue. LinkedIn published its pacing approach in 2014, splitting the day into intervals and allocating against global supply ([LinkedIn, 2014](http://www0.cs.ucl.ac.uk/staff/w.zhang/rtb-papers/linkedin-pacing.pdf)). Yahoo went further a year later: learn the delivery pace from offline and online data, and use a control system to adjust per-group rates, optimizing smoothness and performance together ([Yahoo, 2015](https://arxiv.org/pdf/1506.05851)). 
That's where the first act ends. A system centered on prediction, founded on the cookie, running on second-price auctions. It held for about eight years.
...
Statistics has a name for this. Let W be the highest competing bid. Then `P(win | b) = P(W < b)` is the CDF of W, and the survival function `S(b) = P(W > b)` is its complement. The win rate curve is the survival function of the competing-bid distribution.

In that language, the two auction formats differ as follows. Under second price, the winner pays the second-highest price, so winning reveals W exactly while losing tells you only `W ≥ b`. That is right-censored data. Under first price, the winner pays their own bid, so even winning leaves W unknown, and observation collapses to a binary indicator of whether W falls below b, which survival analysis calls current status data. Strictly less information. The move to first price changed the optimal strategy and also reduced the information available for estimating it.

![Censoring in bid landscape estimation](/images/fig-censoring.png)

*Simulated. The highest competing bid W is drawn from a lognormal distribution with median 2.40 CPM and σ = 0.55, across 400,000 auctions, against a fixed bid of 2.20. Left: 43.7% of those auctions are won. Right: fitting a lognormal to the 174,924 won auctions alone, without accounting for the truncation, gives the dashed curve. At b = 1.5 it puts the win rate at 53% where the true value is 20%. The parameters are chosen for illustration, so the size of the gap isn't a finding; the direction is. Both panels show the second-price case, where winning reveals W. Under first price, winning reveals only that W fell below the bid, and less information is available.*
...
Methods existed already. A 2015 approach used a mixture model: ordinary regression on won samples, censored likelihood on lost ones, weighted by win rate ([Wu et al., 2015](https://dl.acm.org/doi/10.1145/2783258.2783276)). After the shift came general shading frameworks ([Bid shading under first price, 2020](https://dl.acm.org/doi/10.1145/3340531.3412689)), deep distribution networks handling both cases ([Deep distribution network, 2021](https://arxiv.org/pdf/2107.06650)), and distributionally robust approaches ([Distributionally robust bid shading, 2024](https://arxiv.org/pdf/2410.14864)).

These differ in how strong an assumption they make about the distribution or the functional form, not in approach. All of them put censoring into the objective function. Mixture models and Tobit assume a specific distribution. Gradient boosting and deep distribution networks relax the functional form. The end that assumes least — the nonparametric maximum likelihood estimator for current status data, whose solution is an isotonic regression — isn't where the mainstream sits. I return to this below.

### Identity collapsed

The second shock came slower and reached further.

Apple began restricting cross-site tracking in Safari in 2017. GDPR took effect in 2018. In 2021 Apple required explicit consent before tracking. Safari and Firefox moved to blocking third-party cookies by default. Chrome is more complicated: Google repeatedly delayed deprecation, then changed course in 2024, abandoning removal in favor of a user choice mechanism. Third-party cookies haven't disappeared. But they are largely non-functional in Apple's ecosystem, and under regulatory and user-choice pressure their coverage and reliability keep eroding.
...
Responses split two ways. One rebuilds identity: stitch fragments back together from behavioral and deterministic signals into a probabilistic identity graph. That graph is no longer a fact but an estimate, with coverage and error. The other routes around identity: if you can't track individuals, recover population quantities without looking at individuals. Google's Virtual People maps observed device behavior onto a virtual population to estimate deduplicated reach ([Google Research, Virtual People](https://research.google/pubs/cross-media-measurement-with-virtual-people/)). Deduplicating across publishers, where neither side can hand over a user list, calls for noisy sketches, the problem Vector of Counts addresses ([Google Research, Vector of Counts](https://research.google/pubs/pub50153/)). The World Federation of Advertisers has evaluated this class of private estimators systematically ([World Federation of Advertisers, n.d.](https://github.com/world-federation-of-advertisers/cross_media_measurement_project_site/blob/master/public_papers/PRFE_results/Private%20Reach%20&%20Frequency%20Estimators%20Evaluation%20Results.md)).

Together these mark one change: identity went from a given fact to an estimated quantity. Another layer of uncertainty, where none had existed.

### The models got big

The third shock differs in kind. The first two took something away. This one was a capability gained.

From around 2020, deep learning displaced feature engineering and shallow models, and then the models kept growing to foundation-model scale. Per trade press, Meta deployed a retrieval engine called Andromeda in late 2024 to narrow every request to a candidate set from all active ads, and in mid-2025 deployed GEM at the ranking layer, far larger than its predecessors, reporting single-digit percentage lifts in conversion ([Search Engine Land, 2026](https://searchengineland.com/meta-ai-driven-advertising-system-andromeda-gem-468020)). These figures come from the platform via trade media, with experimental conditions and baselines undisclosed.

Bidding underwent a parallel shift. Early work used online reinforcement learning to optimize bidding parameters. Later work treats bidding as sequence generation, using conditional diffusion models ([Generative auto-bidding, 2024](https://dl.acm.org/doi/10.1145/3637528.3671526)) or Decision Transformers over action history and environment state. Recent work targets feasibility under multiple constraints ([Constraint-aware auto-bidding, 2026](https://arxiv.org/html/2602.08261)). The area is active, though for the newest papers I have read only abstracts.
...
But one class of problem didn't move, and in principle will not.

How much to shade under first price still depends on a win-rate curve estimable only from censored data. Attribution still can't produce a correct answer, because the quantity it needs was never in the data. Neither depends on model size.

### Looking back: a spectrum of recoverability

Line up the three shocks and a thread appears.

The first shock's difficulty: you can't observe auctions you lost. The second: you can't observe who is behind the browser. The third, a large increase in model capacity, helps least in exactly those places.

Look further back. Criteo's delayed feedback paper: conversions that haven't happened yet can't be observed at training time. Attribution: the counterfactual is never observed. The feedback loop: labels exist only on impressions you won and served, which you won because of your own policy, so the training data is filtered by yourself.

These are usually lumped together as "data problems." They are not on the same level. The difference is recoverability.
...
Missing counterfactuals are hardest. Not "not observed" but "can't in principle be obtained from observation." The same person can't both see and not see the ad. That quantity requires intervention.

![The four difficulties ordered by recoverability](/images/fig-recoverability.png)

*The four difficulties above, ordered by how much of the missing information can be recovered from data already collected. This ordering is my own framing rather than a standard taxonomy.*

The spectrum has a useful property: the left two fall within survival analysis's range. Delayed feedback is time-to-event modeling, censoring is censored-data estimation, and both have mature toolkits. The right two have no corresponding toolkit, because what they need isn't a better estimator but different data.

Lined up, the conclusion is clear: responsiveness to a bigger model decreases monotonically along the spectrum. Delayed feedback yields to better modeling. Censoring yields to a more correct likelihood. Selection bias yields where there is support. Missing counterfactuals don't yield at all. No model can conjure a world that never happened.

This depends on no performance number. It's a statement about information: you can't extract from data what isn't in the data. Whatever scaling delivered, the right end of the spectrum lies outside its reach.
...
The three shocks share one feature. Rules changed, signals degraded, capabilities improved. But the thing being priced never changed. This system sold one human's attention. Impression, viewability, click-through, conversion, reach: all proxies for that.

That's what is now loosening.
...
The two 402s run on entirely different infrastructure.
...
One is a CDN capability. The other is a startup built for this business. The same status code, two unrelated commercial paths, which suggests this isn't one publisher's experiment. A supply-side market has formed.

### Who gets blocked is a commercial question

Put the three selective publishers side by side and one pattern repeats: Anthropic's and Perplexity's crawlers are blocked or charged; OpenAI's are let through. The same ordering appears at three independent publishers. It maps onto the publicly known landscape of content licensing agreements. And the New York Times, which blocks everyone including OpenAI, maps onto its litigation with OpenAI.

In other words, this young machine-audience market was never an open market. Access runs on bilateral contracts, not bids. A new AI company that wants its crawler to read this content negotiates an agreement; it doesn't post a price.

### Declaration and behavior have come apart

Comparing robots.txt against observed behavior shows a gap.
...
Anthropic isn't on that list. Yet in testing, ClaudeBot received the same markdown and the same ad as the vendors that are.
...
The root problem is that robots.txt is a binary protocol — allow or deny — while reality already has at least four tiers: full, stub, paid, refused. The protocol can't express a price, so the price ended up in an HTTP status code.
...
This isn't an ad pasted into a page. It's a complete ad-serving system, with advertiser, campaign, creative version and per-request impression counting, and with the unit of measurement swapped from people to tokens.

The creative is a series of fact tables rather than copy. Founding year, membership size, certification holders, each row followed by a Source field. This is a format optimized to be quoted by a model, since models prefer to cite structured facts with attribution. It optimizes for something entirely different than an ad written for a person.
...
Machine traffic isn't new. Fake traffic, click farms, bots imitating humans, as old as online advertising, with mature taxonomies and a whole detection industry.
...
The surrounding infrastructure is growing the same way. Cloudflare changed its default to blocking known AI crawlers unless they pay, and built a settlement marketplace ([Cloudflare, 2025](https://blog.cloudflare.com/introducing-pay-per-crawl/); [TechCrunch, 2026](https://techcrunch.com/2026/07/01/cloudflares-new-policy-pushes-ai-companies-to-pay-for-publishers-content/)). The Atlantic's 402 is that system in production. At the protocol layer, two competing frameworks contest the standard for how advertising agents talk to each other ([Ad Context Protocol, n.d.](https://docs.adcontextprotocol.org/docs/faq); [IAB Tech Lab, 2026](https://iabtechlab.com/standards/aamp-agentic-advertising-management-protocols/)), though these remain intentions rather than settled facts.
...
The more important caveat is a misreading waiting to happen: "traffic" here means requests, and requests are not audiences. An agent may issue dozens of requests to read one article and hit a dozen sites to answer one question. Humans generate a different pattern entirely. A thousand times the requests isn't a thousand times the audience. Advertising prices attention and decisions, not packets.
...
There's a specific risk here, with a scope worth stating. Virtual People maps observed behavior onto a virtual population, an imputation under structural assumptions, designed to fill observational gaps. If agent behavior enters the observations, that mapping may project it into "people." And because imputation is the method's job, the contamination need not surface as a detectable outlier.

The limit: this applies mainly to agents that don't declare themselves. For crawlers like ClaudeBot that announce themselves, mainstream pipelines generally filter known crawlers before modeling. I couldn't find preprocessing details for Virtual People, so the honest statement is that the size of this risk depends on how much undeclared automated traffic exists, which is precisely the hardest thing to measure.
...
There are roughly three directions available. This taxonomy is mine, not an industry consensus.
...
This isn't a new problem. It's the invalid traffic problem from the human web, except that here even "is this a real audience" has degraded into a self-declared string. The human web spent twenty years and an entire detection industry failing to solve it.

The machine-audience market didn't route around the old market's difficulty. It inherited the same one.
...
Another way to put it: the fastest-growing part of this industry is winning on information structure, not algorithms.
...
The three shocks passed and the industry answered each. The fourth has barely started, and it differs in kind: the first three asked how to do the job well under worse conditions; this one asks what the job is. That can't be engineered first and understood later.

A few questions look unavoidable. This is a personal selection, not a recognized agenda.

What is the exchange rate between the two currencies? As long as human and machine markets coexist, advertisers must allocate across them, and allocation needs comparable measures. There's neither an exchange rate nor a theory for building one. One route is the shared downstream: both point at conversion, so use incremental conversion as the common unit. That requires reliable incrementality on both sides, which doesn't exist. And there is a prior problem: if impressions on the machine side can be forged freely, the token-denominated half of the currency doesn't stand up in the first place.

What does frequency mean for a model audience? Frequency capping rests on human psychology: diminishing returns, then irritation. If the reader is a model, none of that holds. Is reading the same sponsored passage ten times versus once linear, diminishing, or actually more likely to produce a citation? I couldn't find public research on this, which doesn't mean none exists, only that it isn't widely discussed. Under the dual-track path it has to be answered, or the machine side has reach without frequency and the measurement system is incomplete.

Can randomization become infrastructure? The spectrum points at a practical suggestion: problems at the right end respond only to intervention, so build randomization into delivery, holding out a small fraction of traffic by default, continuously generating counterfactuals, rather than designing an experiment each time one is needed. Technically feasible. The cost is revenue on that traffic; the benefit is estimability across the system. Few do it, not because it can't be done, but because the cost is immediate and the benefit is deferred.

How should censored data be handled under first-price auctions? The observation structure corresponds to current status data, for which mature nonparametric estimation theory exists. But the bid shading literature I have read stays with parametric assumptions or censored regression on tree models; the isotonic route isn't a main line. There may be good engineering reasons, slower convergence and awkward inference among them, or the two fields may have developed separately. My search wasn't deep enough to tell; settling it would take a proper literature review.

Put together, these point the same way. The first three are new problems from a change in who the audience is. The fourth is an old problem from an end of the spectrum that hasn't moved in fifteen years. Answering any of them requires not a larger model, but better experimental design and clearer definitions.
